...
OpenSpending.BubbleTree = function(config, onHover, onUnHover) {
	...
	me.traverse = function(node, index) {
        ...
        node.beautifulNumber = me.ns.Utils.beautifulLongNumber(node.amount);
...
OpenSpending.BubbleTree.Utils.formatNumber = function(n) {
	...
	if (n >= 1000000000000) return prefix+Math.round(n / 100000000000)/10 + ' блн';
	if (n >= 1000000000) return prefix+Math.round(n / 100000000)/10 + ' млрд';
	if (n >= 1000000) return prefix+Math.round(n / 100000)/10 + ' млн';
	if (n >= 1000) return prefix+Math.round(n / 100)/10 + ' хил';
	else return prefix+n;
	
};

OpenSpending.BubbleTree.Utils.beautifulLongNumber = function (nStr)
{
	nStr += '';
	x = nStr.split('.');
	x1 = x[0];
